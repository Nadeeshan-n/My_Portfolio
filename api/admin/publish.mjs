...
const cleanData = (input) => ({
  projectList: Array.isArray(input?.projectList)
    ...
    : [],
  educationList: Array.isArray(input?.educationList)
    ? input.educationList.map((item) => ({
        degree: String(item.degree || ''),
        institutionLink: String(item.institutionLink || ''),
    ...
    : [],
  allSkills: Array.isArray(input?.allSkills)
    ? input.allSkills.map((item) => ({
        name: String(item.name || ''),
        logo: String(item.logo || ''),
      }))
    : [],
  contactLinks: Array.isArray(input?.contactLinks)
    ? input.contactLinks.map((item) => ({
        platform: String(item.platform || ''),
        handle: String(item.handle || ''),
        link: String(item.link || ''),
        icon: String(item.icon || 'Mail'),
      }))
    : [],
});
...
const createDataSource = (data) => {
  ...
  const projects = data.projectList
    .map((project) => {
      const image =
        project.imageKey === 'weather'
          ? 'weatherImg'
          : project.imageKey === 'driveSmart'
            ? 'driveSmartImg'
            : project.imageKey === 'aiAgent'
              ? 'aiAgentImg'
...
export default async function handler(request) {
  ...
  if (request.method === 'GET') {
    return json({ ok: true, message: 'Admin publishing is configured.' });
  }
  ...
  try {
    ...
    const currentResponse = await githubRequest(
      `/repos/${REPO}/contents/${DATA_PATH}?ref=${encodeURIComponent(BRANCH)}`
    );

    if (!currentResponse.ok) {
      const details = await currentResponse.text();
      return json({ error: 'Could not read portfolio data from GitHub.', details }, 502);
    }
    ...
    if (!updateResponse.ok) {
      ...
      return json({ error: 'GitHub rejected the portfolio update.', details }, 502);
    }

    const result = await updateResponse.json();
    return json({
      ok: true,
      message: 'Portfolio published successfully. Vercel will redeploy from the GitHub commit.',
