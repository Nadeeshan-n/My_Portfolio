...
const cleanData = (input) => ({
  ...
  allSkills: Array.isArray(input?.allSkills) ? input.allSkills.map((item) => ({
    name: String(item.name || ''),
    logo: String(item.logo || ''),
  })) : [],
  contactLinks: Array.isArray(input?.contactLinks) ? input.contactLinks.map((item) => ({
...
export default async function handler(req, res) {
  try {
    ...
    if (!currentResponse.ok) {
      ...
      console.error('GitHub read failed:', currentResponse.status, details);
